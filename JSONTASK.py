...
if countries_data:
    # Printing JSON data using 'countries_data' variable
...
def fetch_countries_data(url):
    try:
        ...
        print(f"Error fetching data: {e}")
        return None
#Fetching Country, Currencies & currency symbol
...
def fetch_countries_data(url):
    try:
        ...
        print(f"Error fetching data: {e}")
        return None
#Fetching the countries which have DOLLAR as its currency
def display_countries_with_dollar_currency(country_data):
    for country in country_data:
        name = country.get('name', {}).get('common', 'N/A')
        currencies = country.get('currencies', {})
        
        # Checking if the currency is "Dollar"
...
def fetch_countries_data(url):
    try:
        ...
        print(f"Error fetching data: {e}")
        return None
#Fetching the countries which have EURO as its currency
...
if countries_data:
    ...
    print("Failed to fetch data.")
#Fetching the names of all breweries present in the states of Alaska, Maine and New York
# Example usage
url = "https://restcountries.com/v3.1/all"
countries_data = fetch_countries_data(url)
